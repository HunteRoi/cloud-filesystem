...
internal static class MediatorVerifications
{
    ...
    {
        mediator
            .Verify(mediator => mediator.Send(It.Is<ReadDocumentPropertiesRequest>(request => request.DocumentId == documentId), It.IsAny<CancellationToken>()), Times.Once());
    }

    public static void ReturnedNullForDocument(this Mock<IMediator> mediator, Guid documentId)
    {
        mediator
            .Verify(mediator => mediator.Send(It.Is<ReadDocumentPropertiesRequest>(r => r.DocumentId == documentId), It.IsAny<CancellationToken>()), Times.Once());
    }
}